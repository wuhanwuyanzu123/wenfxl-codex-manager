...
def _smsbower_configured_country_candidates() -> set[int]:
    raw = getattr(cfg, "SMSBOWER_COUNTRY_CANDIDATES", None)
    if raw is None:
        try:
            raw = getattr(cfg, "_c", {}).get("smsbower", {}).get("country_candidates")
        except Exception:
            raw = None
    if raw in (None, ""):
        return set()
...
def _smsbower_pick_country_id(proxies: Any, *, service_code: str, preferred_country: int,
    ...
    allowed = _smsbower_configured_country_candidates()
    ...
    for r in rows:
        ...
        for route in r.get("routes", []) or []:
            try:
                cost = float(route.get("cost", -1.0))
                count = int(route.get("count", 0))
            except Exception:
                continue
            if count <= 0 or cost < 0:
                continue
            if limit_max > 0 and cost > limit_max:
                continue
            if limit_min > 0 and cost < limit_min:
                continue
            valid_routes.append((cost, count))

        if not valid_routes:
            continue

        best_cost = min(cost for cost, _ in valid_routes)
        total_count = sum(count for _, count in valid_routes)
        score = _smsbower_country_score(cid, cost=best_cost, count=total_count, preferred_country=preferred_country)
        if score > -1e8:
            scored.append((score, cid, best_cost, total_count))

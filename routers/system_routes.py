...
def execute_docker_update():
    try:
        ...
        print(f"[{core_engine.ts()}] [??] ?? ????????: {image_name}")
        pull_result = subprocess.run(["docker", "pull", image_name], capture_output=True, text=True, timeout=300)
        if pull_result.returncode != 0:
            detail = (pull_result.stderr or pull_result.stdout or "docker pull failed").strip()
            return {"status": "error", "message": f"??????: {detail[-500:]}"}
        ...
        print(f"[{core_engine.ts()}] [??] ?? ???? Compose ????: {compose_file}")
        subprocess.Popen(update_cmd, shell=True)

        return {
            "status": "success",
            "message": f"???????????? {os.path.basename(compose_file)} ???? 20 ??????..."
        }
